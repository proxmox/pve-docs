...
These three steps are the complete workflow for a normal {pve} cluster. The
helper performs the rotations, safety checks, service restarts, and recovery
after an interruption. Dry runs show a concise plan; add `--verbose` for full
node, keyring, and device inventories.

.Before You Begin
* Upgrade `pve-manager` to 9.2.16 or newer and install the latest Ceph
...
* Run the helper as `root` on one node only. Do not start another migration or
  a rolling restart from the web interface while it runs.

1. Migrate the Cluster-Owned Keys
+++++++++++++++++++++++++++++++++

First review the plan by running the migration script in the default dry-run
mode:
...
This migrates the cluster keys the helper manages: the manager, metadata
server, and OSD keys, the monitor key, the bootstrap and crash keys, and the
xref:pveceph_cephx_lockbox[encrypted OSD lockbox keys]. A daemon that cannot
take its new key while running is restarted, one at a time, monitors included.
It does not change the keys of storage users or of
xref:pveceph_keyrings[client.admin], remove existing tickets early, or disable
the old encryption method. If the run stops, repeat the dry run and apply
command. The helper resumes its recorded work.
...
.Compatibility
A Ceph user such as `client.admin` or the user of a storage is used by one or
more consumers. A consumer is a workload or process, such as a virtual machine
with RBD disks, a container on RBD, a CephFS mount, a {pve} service, or an
external Ceph client.

Migrate a user's key only if every consumer of it supports `aes256k`. Ceph
programs from the updated {pve} packages support it. Kernel RBD and CephFS
clients require a running kernel version 7.0 or newer. Containers on RBD always
use the kernel client. Virtual machines use it when `krbd` is enabled, and
CephFS uses it unless `fuse` is enabled. Check external systems too. If any
consumer is incompatible or unknown, leave that user's key unchanged.

Run the helper without options. It lists the remaining users and the options
that rotate their keys. Users managed outside {pve} are listed separately;
follow that client's supported key-rotation procedure or leave its key
unchanged.
...
Rotate the keys of compatible storage users together with `client.admin`,
which the command line and every storage without its own user share. The
example assumes that the storage IDs `rbd-vm` and `cephfs-backup` have the
**User name** values `rbd-vm` and `cephfs-backup`, respectively, in
**Datacenter -> Storage**. Replace the option values with your storage IDs; the
helper maps each ID to its configured Ceph user.
...
----
/usr/share/pve-manager/migrations/pve-cephx-rotate-service-keys \
    --rotate-storage-key rbd-vm --rotate-storage-key cephfs-backup \
    --rotate-admin-key
----

Normally, every selected key is `staged next to the current key`. Both keys
then work until you confirm the rotation, so consumers can be refreshed at
their own pace. While a key is staged, do not add or downgrade monitors and do
not run `ceph auth` commands for that user.

WARNING: If the plan says `replaced at once` for any selected user, stop all of
that user's consumers before applying the plan, or leave its key unchanged.
The old key stops authenticating as soon as it is replaced. Existing monitor
tickets expire within three days by default, a reconnect can fail earlier, and
existing data connections can work longer and then fail.

After handling any `replaced at once` warning as described above, apply the
same selection:
...
/usr/share/pve-manager/migrations/pve-cephx-rotate-service-keys \
    --rotate-storage-key rbd-vm --rotate-storage-key cephfs-backup \
    --rotate-admin-key --apply
----

[[pveceph_cephx_refresh_consumers]]
.Refresh Consumers
After applying a client-key rotation, refresh its consumers:
...
* The helper remounts the CephFS storages itself. Unmount a mount it reports
  as busy once nothing uses it; it comes back by itself.
* Update saved keys outside {pve}, then restart or remount external clients.

The helper reports recorded clients that might still hold an old key, but it
cannot tell which key they loaded or see disconnected clients. Check the
complete environment yourself. Then run another dry run and copy the exact
confirmation command it prints. For a staged key, confirmation commits the new
key and drops the old one. For a key replaced at once, it only records that
every consumer was refreshed.

[[pveceph_cephx_finish]]
3. Finish or Pause the Migration
++++++++++++++++++++++++++++++++

After refreshing all consumers, run another dry run. It names any recorded
client that might still hold an old key. Once none is left and you have checked
disconnected and external consumers yourself, it prints the finishing command
for your cluster. If every open rotation is ready, one option confirms all of
them. The command also restricts the ciphers if no key still depends on the old
cipher.
...
Run the exact command the dry run prints. For the example above, when every
open rotation is ready and no key needs the old cipher, it is:
...
For an intentional partial batch, the helper instead prints
`--confirm-clients-refreshed <USER>` for each ready user. The aggregate option
refuses the whole request if any open rotation is not ready.

If any client remains incompatible, do not restrict the ciphers. Leave its key
unchanged and keep the old cipher enabled. The cluster-owned migration has
still removed the 'HEALTH_ERR' condition. You can mute the remaining warnings
as described in xref:pveceph_mute_health_check[Mute a Health Check].

NOTE: `/etc/pve/priv/cephx-key-migration.json` records progress and contains
secret old keys. Protect it, and delete it only after the migration is finished
and access and health are verified: deleting it earlier loses the rotation
records.

Additional Migration Details
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The migration steps above contain the complete normal workflow. The following
sections explain individual health checks, provide more examples, and cover
uncommon recovery cases.
...
|`AUTH_INSECURE_CLIENT_KEY_TYPE` |The key of a Ceph user uses the old cipher.
|Rotate the keys of compatible users, or mute the warning.
|`AUTH_INSECURE_ROTATING_SERVICE_KEY_TYPE` |A rotating service key uses the old
cipher. |Wait a few hours. It clears automatically.
...
The monitors keep a staged key next to the current one and stop promoting
pending Ceph user keys automatically. An older monitor would promote a staged
key on its first use and end the grace period for the whole cluster. The helper
therefore stages only when every monitor supports the feature and checks this
again on every run.

`--confirm-clients-refreshed <USER>`:: With `--apply`, commit one staged key
and drop the previous one after its consumers were refreshed.

`--confirm-all-clients-refreshed`:: With `--apply`, do the same for every open
rotation. This fails before committing any key unless all open rotations are
ready.

`--abort-staged-key <USER>`:: With `--apply`, drop the key the helper staged
for that user and put its current key back into every copy {pve} keeps. A
consumer that already switched to the staged key loses its access.
...
An interrupted helper run is normally resumable. Run the helper again without
`--apply` first. It reads its journal, checks the current cluster state, and
prints the next command. Keep the journal unchanged and use that command rather
than starting a manual rotation.
