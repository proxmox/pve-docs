...
Use the helper that {pve} ships for this. It checks that the monitors accept
the new cipher, rotates the key of every MGR, MDS, and OSD one daemon at a
time, writes the new key where that daemon reads it from, and switches the
service tickets over at the end. It runs as a dry run and shows you the plan,
...
the whole cluster from that node. A second run with `--apply` on any node waits
for one in progress, through a lock on the cluster file system, and gives up
after a few minutes. The rolling restart in the web interface is only kept off
by an advisory lock, as Ceph's configuration store has no compare-and-swap
operation, so do not start one until the run finishes.
...
What the helper leaves to you is `auth_preferred_cipher`, the cipher the
monitors use for every key created without an explicit `--key-type`. It
applies to client and service keys alike, so set it once every client of this
cluster can use the new cipher:
...
A manager or metadata server created afterwards then gets the new cipher too,
instead of raising the error again. Leave it at `aes` while you still have to
create keys for kernel clients that do not support `aes256k`, and run the
helper again for a daemon added later. OSDs are unaffected, as `ceph-volume`
generates their keys locally and already uses the new cipher. The helper sets
the option to `aes256k` itself while it stages pending keys, as Ceph cannot pick
a cipher for those, and puts the previous value back when the run ends; after a
hard kill, the next run with `--apply` does.

A run without further options clears both errors: it rotates the service keys
and switches the service tickets over. The options below cover the warnings
that remain and can be combined into a single run, which the helper carries
out in the order Ceph needs: the monitor key, the service daemons, the client
and lockbox keys, the service tickets, and the rotating keys. Pass `--help` for
the full list.

`--only <SCOPE>`:: Restrict the run to `mon`, to one daemon type, such as
`--only mgr`, or to a single daemon, such as `--only osd.3`. Several can be
given at once, either comma-separated or by repeating the option. Use this to
try the migration on a few daemons first, or to repair a daemon that was left
behind. A restricted run does not switch the service tickets over, as it cannot
know that the keys it skipped are migrated. This only scopes the daemon keys and
the `mon.` key. A client key is rotated whenever its own option asks for it.

`--rotate-mon-key`:: Also rotate the shared `mon.` key, which restarts every
monitor, one at a time. Ceph does not report that key, as it lives in the
monitor keyrings rather than in the authentication database, so it is not needed
to clear either error; it is the most privileged key in the cluster though. Once
rotated, the key sits in the authentication database and Ceph reports it like
any other, and from then on this option is required.

`--restart-daemons`:: Stop, rotate, and start each daemon instead of handing it
the new key while it runs. The helper falls back to this on its own for a daemon
that does not take the key that way, for a standby manager, and for a daemon an
earlier run left stopped, so the `noout` flag is set for the run's OSDs either
way. Monitors always restart, as they have no `rotate-key` command.
...
After a successful unrestricted run, `ceph -s` no longer reports
`AUTH_INSECURE_SERVICE_KEY_TYPE` or `AUTH_INSECURE_SERVICE_TICKETS`.
...
Rotate the Service Keys by Hand
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
service and client key is migrated, drop the old cipher to clear both:
...
WARNING: Only run this after `ceph -s` reports neither
`AUTH_INSECURE_SERVICE_KEY_TYPE` nor `AUTH_INSECURE_CLIENT_KEY_TYPE`. Any key
that still uses the old cipher can no longer authenticate afterwards, and that
includes `client.admin`, which locks you out of the command line. To recover,
...
`AUTH_EMERGENCY_CIPHERS_SET` warning for as long as it is in place, so remove it
again once the keys are sorted out.
