...
Ceph authenticates its daemons and clients with cephx keys, which are stored in
keyring files. {pve} keeps the keyrings of a managed cluster in two places, and
the difference matters whenever a key changes.

The authoritative `client.admin` keyring is
`/etc/pve/priv/ceph.client.admin.keyring`, not the one in `/etc/ceph`. {pve}
stores it on the xref:chapter_pmxcfs[configuration file system (pmxcfs)], which
syncs to every node in real time. Write it once on any node, no copying is
needed. Keyrings for external Ceph clusters are stored in
`/etc/pve/priv/ceph/<STORAGE_ID>.keyring`.

The per-daemon keyrings below `/var/lib/ceph` are local to their node and are
not synced. Update them on the node the daemon runs on.

WARNING: The file `/etc/pve/priv/ceph.mon.keyring` holds the `mon.` key as well,
and `pveceph mon create` passes that file to `ceph-mon --mkfs`. If you change
the `mon.` key without updating that file, every monitor created later boots
with a stale key and can never join the cluster. Always merge a new key into
that file, never overwrite it, as it also holds the `client.admin` key.
...
WARNING: Do not rotate a `client.osd-lockbox.<UUID>` key by hand. Such a key
has two copies that have to agree: the entry in the authentication database,
and the `ceph.cephx_lockbox_secret` LVM tag on the OSD's block device, which
`ceph-volume` rebuilds the lockbox keyring from every time it activates the
OSD. Rotating only the entry leaves that OSD unable to unlock at its next
start.

Migrate these keys with `--rotate-lockbox-keys`, which writes both copies. Run
a dry run first, then repeat it with `--apply` after reviewing the plan:
...
The OSDs keep running, as nothing reads the lockbox keyring while an OSD runs.
The new key is staged first, so an interrupted run leaves the OSD able to
unlock; run the helper again to finish it. A key that was already rotated by
hand is brought back into agreement the same way. If the block volume of an
OSD cannot be found, the helper names that OSD and refuses to start.

The helper leaves a lockbox entry alone when no OSD in the current map has its
UUID. Do not remove the entry based on that fact alone. Stop OSD provisioning,
confirm from a fresh map that the UUID is absent, and check every node for an
LVM or raw `ceph-volume` device with that UUID. Save the auth entry before you
remove it. If any node is unavailable or the device might return, keep the
entry; it does not affect current OSDs.

Migrate rather than mute: the only capability of such a key returns the LUKS
passphrase of its OSD, so anyone who can read cephx traffic could decrypt that
disk with it. An existing OSD keeps the key it was created with, so every
encrypted OSD from before the upgrade is affected.
