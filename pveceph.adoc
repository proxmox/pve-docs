...
A stopped daemon is migrated as well, from what its node reports about itself,
and left stopped. Otherwise it would block the service ticket switch at the end.
...
Then, for the OSD with the ID 3, on its node:
...
NOTE: A stopped OSD keeps its block device open for a moment longer, so
`set-label-key` can fail with a device busy error right after `systemctl stop`.
Wait a few seconds and run it again. Do not start the OSD before the label is
written, as its key in the authentication database has already changed.

Check the label before you rebuild the data directory from it. `whoami` has to
be the ID of this OSD and `ceph_fsid` the FSID of this cluster. If they are not,
the device belongs to another OSD, and priming it would overwrite that OSD's
identity as well as its key:
...
Migrate these keys with `--rotate-lockbox-keys`, which writes both copies:
