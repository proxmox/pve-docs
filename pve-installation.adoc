...
{pve} is based on Debian GNU/Linux. The install disk images (ISO files)
...
run this option.
+
NOTE: This entry is available on x86 installation media only, as `memtest86+`
...
[thumbnail="screenshot/pve-install-summary.png", float="left"]
...
https://pve.proxmox.com/wiki/Automated_Installation[Visit the wiki] for more
...
{webwiki-url}Install_Proxmox_VE_on_Debian_13_Trixie[wiki].
...
See the list of all official tutorials on the
...
* link:/wiki/Install_Proxmox_VE_on_Debian_13_Trixie[Install Proxmox VE on Debian 13 Trixie]
