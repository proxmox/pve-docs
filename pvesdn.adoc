...
The example assumes a 3-node Proxmox cluster ('sdn1', 'sdn2' and 'sdn3') with
...
`203.0.113.0/24` subnet will be routed via the external WireGuard peer,
`198.51.100.10`.

Set Up WireGuard Fabric
^^^^^^^^^^^^^^^^^^^^^^^
...
'Fabrics' and create a new WireGuard fabric by clicking 'Create' and selecting
'WireGuard'.
...
this node, `192.0.2.1` in the case of sdn1. Since no additional subnets should
be routed via this node in this example, leave the 'Allowed IPs' field empty.
...
select type 'External' in the node creation dialog. Enter the public key of
