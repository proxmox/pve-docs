...
NOTE: Subscription keys are bound to the host architecture. An arm64 host needs
a dedicated arm64 subscription key; an x86 key is not valid on it, and vice
versa.
...
[[sysadmin_dbgsym_repo]]
{pve} Debug Symbol Repository
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
and Trixie based {pve} releases (and newer).

.File `/etc/apt/sources.list.d/pve-dbgsym.sources`
----
Types: deb
